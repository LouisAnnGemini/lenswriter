...
*   **章节归档 (Archive)**：
    *   **归档操作**：在左侧章节目录中，点击章节右侧的**归档图标 (Archive)**，或者在编辑器顶部章节标题旁点击归档按钮，即可将该章节归档。
    *   **阅读模式**：归档后的章节将进入**只读模式**。编辑器顶部会显示归档提示，所有文本块、标题、描述均不可编辑，且无法添加或删除块、场景。
    *   **隐藏显示**：归档章节在左侧目录中默认隐藏，保持界面整洁。
    *   **查看归档**：点击左侧目录底部的 **Show Archived (眼睛图标)** 按钮，可以显示已归档的章节。归档章节会以半透明状态显示在原有位置，保持原有的章节顺序。
    *   **取消归档**：在显示归档章节的状态下，点击章节右侧的**取消归档图标 (ArchiveRestore)**，即可恢复该章节的编辑功能。
    *   *注：归档章节及其内部的场景无法进行拖拽排序。*
...
*   **置顶透镜 (Pinned Lenses)**：
    *   **置顶操作**：在任意 Lens 卡片或详情面板中，点击**图钉图标 (Pin)** 即可将该 Lens 置顶。
    *   **独立展示**：被置顶的 Lens 会固定显示在 Lenses 界面的最上方，形成一个单独的 "Pinned Lenses" 栏目。该栏目采用了更宽的卡片布局，方便您快速查看和访问最重要的设定或线索。
    *   **取消置顶**：点击置顶卡片上的**取消图钉图标 (PinOff)** 即可解除置顶，该 Lens 会回到下方的 "All Lenses" 列表中。
*   **瀑布流展示 (All Lenses)**：未置顶的 Lens 会在下方以瀑布流卡片形式展示，包含位置信息（章节 > 场景）。
*   **筛选功能**：
...
LensWriter 采用了本地优先的架构设计，确保您在任何网络环境下都能流畅创作。

*   **本地存储优先**：所有创作数据都会实时保存到浏览器的本地存储 (`localStorage`) 中。这意味着即使在没有网络连接的情况下，您也可以正常进行写作、修改和整理，不会丢失任何进度。
*   **Firebase 实时同步**：当您登录并处于联网状态时，应用程序会自动将本地数据与 Firebase 云端数据库进行实时同步。
*   **无缝切换**：无论您是离线创作还是多设备切换，系统都会自动处理数据冲突与同步，确保您的数据始终保持最新。
